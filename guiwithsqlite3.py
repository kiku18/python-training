...
def Runv():
    ...
    if unch==1:
        ...
        conn=sqlite3.connect(r'C:\Users\kumar\mydatabase.db')
        cur=conn.cursor()
        sql=f"insert into mytable values({num1.get()},{num2.get()},'{operation}','{result.get()}')"
        print(sql)
        cur.execute(sql)
        conn.commit()
        cur.close()
...
num2.set('')
result.set('')
Checkbutton(root,text=" Save ton file",variable=chk).grid(row=3,column=0)
Checkbutton(root,text=" save to database",variable=unchk).grid(row=3,column=1)
